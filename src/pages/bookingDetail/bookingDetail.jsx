...
const getSimilarTours = async (tourId) => {
  try {
    ...
    console.log('Fetching similar tours for tourId:', tourId);
    if (!token) {
      message.error('Vui lòng đăng nhập để xem tour tương tự');
      return [];
    }
    const response = await fetch(`http://localhost:8080/api/tours/${tourId}/similar`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Lỗi khi lấy tour tương tự: ${response.status}`);
    }
    const data = await response.json();
    console.log('Similar Tours:', data);
    return data;
  } catch (error) {
    console.error('Lỗi getSimilarTours:', error.message);
    return [];
  }
};

const RatingModal = ({ visible, onCancel, onConfirm, loading }) => {
...
const CancelModal = ({ visible, onCancel, onConfirm, loading, cancellationInfo }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [
          ...
          axios.get(
            `http://localhost:8080/api/reviews/by-tour/${state.id}?page=${currentPage - 1}&size=${pageSize}`,
            { headers: { Authorization: `Bearer ${token}` } }
        ...
        if (tourId) {
          ...
          console.warn('Không tìm thấy tourId trong bookingResponse:', bookingResponse.data);
        }
      } catch (error) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="absolute bottom-6 left-6 right-6">
            ...
            <Text className="text-white/90"><span className='text-white '>{tour.location || 'N/A'}</span></Text>
          </div>
        </motion.div>
                ...
                <div>
                  <Text className="text-sm text-gray-600">
                    ...
                    {bookingDetail.bookingDate
                      ? format(new Date(bookingDetail.bookingDate), 'dd/MM/yyyy', { locale: vi })
                      : 'N/A'}
                  </Text>
                  <Text className="text-sm text-gray-600 block mt-3">
                    ...
                    {tour.tourDetails?.[0]?.startDate
                      ? format(new Date(tour.tourDetails[0].startDate), 'dd/MM/yyyy', { locale: vi })
                      : 'N/A'}
                  </Text>
                  <Text className="text-sm text-gray-600 block mt-3">
                  ...
                  <div key={index} className="space-y-3">
                    <Text className="text-sm text-gray-600">
                      ...
                      {detail.startDate && detail.endDate
                        ? `${format(new Date(detail.startDate), 'dd/MM/yyyy', { locale: vi })} - ${format(
                        ...
                        children: (
                          <>
                            ...
                            <Text className="text-xs text-gray-500 block">
                              {format(new Date(item.changeDate), 'dd/MM/yyyy HH:mm', { locale: vi })}
                            </Text>
                            {item.reason && (
                      ...
                      description={
                        <>
                          ...
                          <Text className="text-xs text-gray-500 block mt-1">
                            {format(new Date(review.reviewDate), 'dd/MM/yyyy', { locale: vi })}
                          </Text>
                        </>
                      }
                    />
