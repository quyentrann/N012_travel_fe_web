...
const SearchPage = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { filteredTours, loading } = useSelector((state) => state.tours);
  ...
  // Fetch tours hoặc search history khi component mount
  useEffect(() => {
    const fetchData = async () => {
      setLoadingTours(true);
      try {
        if (isAuthenticated) {
          const historyTours = await getSearchHistory();
          setTours(historyTours);
          dispatch(setFilteredTours(historyTours));
        } else {
          const allTours = await getTours();
          setTours(allTours);
          dispatch(setFilteredTours(allTours));
        }
      } catch (error) {
